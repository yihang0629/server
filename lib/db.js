...
let users = `create table if not exists users(
    ...
    createTime VARCHAR(100) COMMENT '注册时间',
    status INT DEFAULT 1 COMMENT '状态 0-禁用 1-正常',
    role INT DEFAULT 0 COMMENT '角色 0-普通用户 1-管理员',
    avatar INT DEFAULT 0 COMMENT '头像索引'
);`
...
// 新增 注册用户到users表（同步用户信息）
exports.regUserToUsers = (username) => {
    let _sql = "insert into users (username, email, phone, createTime, status, role, avatar) values (?, ?, ?, ?, ?, ?, ?);"
    let now = new Date().toLocaleString('zh-CN');
    return query(_sql, [username, '', '', now, 1, 0, 0])
}
...
// 分页查询用户列表（从user表）
exports.findUserPage = (page, pagesize, username, nickname, user_pic) => {
    let conditions = [];
    ...
    if (user_pic !== '' && user_pic !== null && user_pic !== undefined) {
        conditions.push(`user_pic = "${user_pic}"`);
    }

    let whereClause = conditions.length > 0 ? 'WHERE ' + conditions.join(' AND ') : '';
    let countSql = `SELECT COUNT(*) as total FROM user ${whereClause};`;
    let dataSql = `SELECT * FROM user ${whereClause} ORDER BY id DESC LIMIT ${(page - 1) * pagesize},${pagesize};`;

    return Promise.all([query(countSql), query(dataSql)]);
}

// 更新用户状态（从user表）
exports.updateUserStatus = (id, nickname) => {
    let _sql = `UPDATE user SET nickname = "${nickname}" WHERE id = ${id};`;
    return query(_sql);
}
